...
namespace Dbscan
{
  ...
  public delegate decimal DistanceFunction(in IPointData p1, in IPointData p2);

  internal static class DistanceFunctions
  {
    public static decimal EuclideanDistance(in IPointData a, in IPointData b)
    {
      var xDist = b.Point.X - a.Point.X;
      var yDist = b.Point.Y - a.Point.Y;
      return (decimal)Math.Sqrt((double)(xDist * xDist + yDist * yDist));
    }

    public static decimal PercDistance(in IPointData a, in IPointData b)
    {
      return Math.Abs((a.Point.Y - b.Point.Y) / (a.Point.Y));
    }
  }
  ...
  public class ListSpatialIndex<T> : ISpatialIndex<T> where T : IPointData
  {
    ...
    public ListSpatialIndex(IEnumerable<T> data)
        : this(data, DistanceFunctions.PercDistance) { }
    ...
    /// </returns>
    public IReadOnlyList<T> Search(in IPointData p, decimal epsilon)
    {
      var l = new List<T>();
